...
int
md5(char *md5str, unsigned char *str, int slen)
{
    MD5Context context;
    unsigned char buff[16];
    md5_init(&context);
    md5_update(&context, str, slen);
...
int
md5_file (char * filename, char *bmd5, int bsize)
{
    FILE *file;
    MD5Context context;
    unsigned char buff[16];
    int i,len;
    if (bsize != 16 && bsize != 32) {
        return -1;
    }
    unsigned char buffer[8086];
    if (!(file = fopen (filename, "rb"))) {
        //printf ("%s can't be opened\n", filename);
        return -2;
    } else {
        md5_init (&context);
        while ((len = fread (buffer, 1, 8086, file)) > 0)
        ...
        if (bsize == 16) {
            ...
            int j = 0;
            for (i = 0; i < 16; i++) {
                sprintf(&bmd5[j], "%x", (buff[i] & 0xF0) >> 4);
                j++;
                sprintf(&bmd5[j], "%x", (buff[i] & 0x0F));
                j++;
            }
        }
    }
